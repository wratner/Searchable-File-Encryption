...
public class BlindStorage {
	...
	private void initBlocks() {
		...
		for (int i = 0; i < totalSize; i++) {
			try {
				...
				// FileOutputStream blockOutputStream = new FileOutputStream(DIR
				// + i);
	...
	/*
	 * Creates blocks from file chunks by adding headers and DOES NOT encrypt
	 * them.
	 * 
	 * HEADER FORMAT: ??????
	 */
	private List<byte[]> createBlocks(List<Byte> inputBytes, String message_id) {
		String header;
		Integer sizef = ((Double) Math.ceil((double) inputBytes.size()
				/ (double) BLOCK_SIZE)).intValue();
		int start = 0;
		...
		try {
			for (start = 0; start < inputBytes.size(); start = stop) {
				...
				// Take into account the header when getting the parts of the
				// file that we need
	...
	/*
	 * gets the placement of blocks using the filename as a hashed seed to a
	...
	private List<Integer> getLocations(String fileName, Integer max) {
		...
		try {
			...
			final byte[] mac_data = sha256_HMAC.doFinal(asciiCs
					.encode(fileName).array());
	...
	/*
	 * Returns the blocks Identified by blockIds. This reads from the
	 * filesystem.
	 */
	...
	/*
	 * Writes the blocks out to disk by first checking to see if a block is
	 * occupied before tyring to write it, if it is occupied, it tries to put
	 * the block in the next place in the list of locations
	 * 
	 * TODO: ADD ENCRYPTION EITHER HERE OR SOMEWHERE ELSE
	 * 
	 * QUESTION: Paper says to check for something and abort, need to look at
	 * this
	 */
	private boolean writeBlocks(List<byte[]> blocks, Integer[] locations) {
		int prgIndex = 0;
		// Perform our checks
		if (!(sizefOneBlockSuccess(locations) && allGood(locations,
				blocks.size()))) {
			return false;
		}
	...
	/*
	 * Returns true if there is at least 1 block free in the first kappa blocks.
	 */
	...
	/*
	 * returns true if sizef locations are free The chance of this returning
	 * false is suppose to be negligible.
	 */
	...
	/*
	 * Takes a list of blocks and a message id and puts back together the
	 * original message
	 */
		...
		for (String s : blocks) {
			if (s.contains(messageId)) {
				if (Character.isDigit(s.charAt(0))) {
					if (s.indexOf(":") == 1) {
						sizef = Character.getNumericValue(s.charAt(0)); // NEED
																		// THIS
																		// FOR
																		// LATER
					}
	...
	public boolean addFile(File file) {
		...
		List<Integer> locations = getLocations(file.getName(),
				maxSize(chunks.size()));
		Integer[] intLocations = Arrays.copyOf(locations.toArray(),
				locations.size(), Integer[].class);
	...
	public String getFile(String messageId) {
		...
		if (maxSize(sizef) != kappa) {
			...
			intLocations = Arrays.copyOf(locations.toArray(), locations.size(),
					Integer[].class);
	...
	public boolean testFile(File file) {
		...
		decryptedMessage = buildMessage(decryptBlocks(encryptedBlocksFromFile),
				file.getName());
		System.out
				.println("***********************MESSAGE***********************");
